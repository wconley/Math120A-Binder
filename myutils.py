...
def pgfplots_dump(arr, filename, headers=None):
    """Print the numpy array "arr" as a PGFPlots table"""

    with open(filename, "w") as outfile:
        if headers is not None and len(headers) == arr.shape[1]:
            print("\t".join(headers), file=outfile)
        for line in arr:
            print("\t".join(line.astype("str")), file=outfile)
